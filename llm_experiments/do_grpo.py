...
def _do_update(optimizer, noiser_params, params, is_input_token, generations, raw_scores, lr):
    ...
    def update_loop(state, x):
        ...
        loss, grad = fast_batch_grad(new_params, params, noiser_params, is_input_batch, gen_batch, score_batch)
    ...
    (new_params, optimizer), losses = jax.lax.scan(update_loop, (params, optimizer), (
        is_input_token.reshape(args.num_minibatches, args.total_parallel_generations // args.num_minibatches, -1),
        generations.reshape(args.num_minibatches, args.total_parallel_generations // args.num_minibatches, -1),
        true_scores.reshape(args.num_minibatches, args.total_parallel_generations // args.num_minibatches)))
...
do_update = jax.jit(shard_map(
    ...
    out_specs=(P(), P(), P(), P())
), donate_argnums=(0, 1, 2)).lower(optimizer, noiser_params, params, jnp.zeros((args.total_parallel_generations, args.generation_length), dtype=jnp.bool), jnp.zeros((args.total_parallel_generations, args.generation_length), dtype=jnp.int32), jnp.zeros(args.total_parallel_generations), args.lr).compile()
print("Compile time", time.time() - start_time)
print("memory info")
